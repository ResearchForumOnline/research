...
SRC = ROOT / "papers" / "zmail-callchat-protected-communications-1.0.md"
OUTDIR = ROOT / "publication_work" / "protected-communications-1.0"
DOCX = OUTDIR / "Zmail-CallChat-Protected-Communications-1.0-Shafaet-Brady-Hussain.docx"
PDF = OUTDIR / "Zmail-CallChat-Protected-Communications-1.0-Shafaet-Brady-Hussain.pdf"
...
def build_docx(blocks):
    ...
    set_font(header.add_run("ZMAIL + CALLCHAT | PROTECTED COMMUNICATIONS 1.0"),size=8,color=MUTED)
...
def build_pdf(blocks):
    ...
    def footer(canvas,doc):
        canvas.saveState(); canvas.setFont("Helvetica",7.5); canvas.setFillColor(colors.HexColor("#595959")); canvas.drawString(inch,.55*inch,"ZMAIL + CALLCHAT | PROTECTED COMMUNICATIONS 1.0"); canvas.drawRightString(7.5*inch,.55*inch,f"Shafaet Brady Hussain | {doc.page}"); canvas.restoreState()
    pdf=SimpleDocTemplate(str(PDF),pagesize=letter,rightMargin=inch,leftMargin=inch,topMargin=.85*inch,bottomMargin=.8*inch,title="Zmail and CallChat 1.0: Layered Protected Communications Across Email, Messaging, and Calls",author="Shafaet Brady Hussain",subject="Protected email, Matrix messaging, call-media key mixing, evidence boundaries, and reproducibility")
