...
SRC = ROOT / "papers" / "zmath-shield-zme1-evidence-containers-1.0.md"
OUTDIR = ROOT / "publication_work" / "zme1-1.0"
DOCX = OUTDIR / "ZMath-Shield-ZME1-Evidence-Containers-1.0-Shafaet-Brady-Hussain.docx"
PDF = OUTDIR / "ZMath-Shield-ZME1-Evidence-Containers-1.0-Shafaet-Brady-Hussain.pdf"
...
def build_docx(blocks):
    ...
    set_font(header.add_run("ZMATH SHIELD | ZME1 1.0 | PUBLIC PREPRINT"),size=8,color=MUTED)
...
def build_pdf(blocks):
    ...
    def footer(canvas,doc):
        canvas.saveState(); canvas.setFont("Helvetica",7.5); canvas.setFillColor(colors.HexColor("#595959")); canvas.drawString(inch,.55*inch,"ZMATH SHIELD | ZME1 1.0 | PUBLIC PREPRINT"); canvas.drawRightString(7.5*inch,.55*inch,f"Shafaet Brady Hussain | {doc.page}"); canvas.restoreState()
    pdf=SimpleDocTemplate(str(PDF),pagesize=letter,rightMargin=inch,leftMargin=inch,topMargin=.85*inch,bottomMargin=.8*inch,title="ZMath Shield and ZME1 1.0: Authenticated Evidence Containers with Explicit Security Boundaries",author="Shafaet Brady Hussain",subject="Authenticated container specification, public implementation evaluation, threat model, and reproducibility")
